...
const (
	...
	sqlGetZ3XsDdThDt = "" +
		"declare @lsh varchar(255) " +
		"select top 1 @lsh=thdlsh from z3xsddthsyt order by thdlsh asc " +
		"select a.thdmxhh,b.thdjh as thddjh,b.thddlsh as thdddlsh,b.thlsh as thdlsh,a.thddjlx, " +
		"	a.thdyyr,a.thdjfmdid,a.thdckid,a.thdkhppid,c.ddphbj as thphbj, " +
		"	c.ddkhid as thkhid,a.thdhpid,a.thddwid,a.thdhsl,a.thddzxsl, " +
		"	a.thddbqjxj,a.thddcjjxj,a.thdjmsl,a.thdbqjxj,a.thdcjjxj, " +
		"	b.thzdrid,b.thzdsj,b.thbz " +
		"from z3xsddthdt a " +
		"inner join z3xsddtht b on left(a.thdmxhh,12) = b.thlsh and b.thlsh = @lsh " +
		"inner join z3ddt c on c.ddlsh = b.thddlsh"
	sqlDelZ3XsDdThDtSy = "" +
		"delete from z3xsddthsyt " +
		"where thdlsh = ?"
)
...
func (r *repMd) DelZ3XsCkDtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDeleteZ3XsCkDtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) DelZ3XsThtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3XsThtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) DelZ3MdDbCkDtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3MdDbCkDtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) DelZ3MdDbTzDtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3MdDbTzDtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) DelZ3HpCkDjDt(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3HpCkDjDtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) DelZ3HpRkDjDtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3HpRkDjDtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) DelZ3PkDjDtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3PkDjDt, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) DelZ3MdPsTzDtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3MdPsTzDtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
...
func (r *repMd) GetZ3XsDdThDt() ([]*object.Z3XsDdThDt, error) {
	...
	var thdMxHh, thdDjh, thdDdLsh, thdLsh, thBz string
	var thdDjLx, thdJfMdId, thdCkId, thdKhPpid, thPhBj, thKhId, thdHpId, thdDwId, thZdrId int
	var thdYyr, thZdSj time.Time
	var thdHsl, thdDZxSl, thDdBqjXj, thDdCjjXj, thdJmSl, thdBqjXj, thdCjjXj float64
	rList := make([]*object.Z3XsDdThDt, 0)
	for rows.Next() {
		err := rows.Scan(&thdMxHh, &thdDjh, &thdDdLsh, &thdLsh, &thdDjLx,
			...
			&thZdrId, &thZdSj, &thBz)
		if err != nil {
			errMsg := fmt.Sprintf("%s read data err: %s", "GetZ3XsDdThDt", err.Error())
			log.Error(errMsg)
			return nil, errors.New(errMsg)
		}
		rList = append(rList, &object.Z3XsDdThDt{
			ThdMxHh:   thdMxHh,
			ThdDjh:    thdDjh,
			ThdDdLsh:  thdDdLsh,
			ThdLsh:    thdLsh,
	...
	if rows.Err() != nil {
		errMsg := fmt.Sprintf("%s read data err: %s", "GetZ3XsDdThDt", rows.Err().Error())
		log.Error(errMsg)
		return nil, errors.New(errMsg)
	}
	return rList, nil
...
func (r *repMd) DelZ3XsDdThDtSy(id string) error {
	err := goToolMSSqlHelper.SetRowsBySQL2000(r.dbConfig, sqlDelZ3XsDdThDtSy, id)
	if err != nil {
		log.Error(err.Error())
		return err
	}
